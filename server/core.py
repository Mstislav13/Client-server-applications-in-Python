...
class MessageProcessor(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                if self.clients:
                    query_data_list, self.listen_sockets, self.error_sockets = select.select(self.clients,
                                                                                             self.clients, [], 0)
            ...
            if query_data_list:
                for client_with_message in query_data_list:
                    try:
                        ...
                        logger.debug(f'Получение данных из исключения.', exc_info=err)
    ...
    def process_message(self, message):
        ...
        :return:
        """
        if message[WHITHER] in self.names and self.names[message[WHITHER]] in self.listen_sockets:
            try:
        ...
        elif message[WHITHER] in self.names and self.names[message[WHITHER]] not in self.listen_sockets:
            logger.error(
                f'Связь с клиентом {message[WHITHER]} была потеряна. Соединение закрыто, доставка невозможна.')
