...
import hmac
import binascii
from common.descriptor import Port, Address
from common.variables import MAX_CONNECTIONS, WHITHER, SENDER, PRESENCE, \
    ACTION, USER, TIME, MESSAGE, MESSAGE_TEXT, RESPONSE_200, RESPONSE_400, \
    ERROR, ACCOUNT_NAME, EXIT, GET_CONTACTS, RESPONSE_202, LIST_INFO, \
...
class MessageProcessor(threading.Thread):
            ...
            if recv_data_lst:
                for client_with_message in recv_data_lst:
                    try:
                        ...
                        logger.debug(f'Получение данных из исключения.',
                                     exc_info=err)
    ...
    def init_socket(self):
        ...
        logger.info(
            ...
            f'Если адрес не указан, принимаются '
    ...
    def process_message(self, message):
        ...
        :return:
        """
        if message[WHITHER] in self.names and self.names[message[WHITHER]
                                                         ] in self.listen_sockets:
            try:
                send_message(self.names[message[WHITHER]], message)
                logger.info(
                    f'Отправлено сообщение пользователю {message[WHITHER]} '
                    f'от пользователя {message[SENDER]}.')
            except OSError:
                self.remove_client(message[WHITHER])
        elif message[WHITHER] in self.names \
                and self.names[message[WHITHER]] not in self.listen_sockets:
            logger.error(
                f'Связь с клиентом {message[WHITHER]} была потеряна. '
                f'Соединение закрыто, доставка невозможна.')
            self.remove_client(self.names[message[WHITHER]])
        else:
            logger.error(
                f'Не зарегистрированный клиент: {message[WHITHER]}. '
                f'Нельзя отправить сообщение.')
    ...
    def process_client_message(self, message, client):
        ...
        # Если это сообщение, то отправляем его получателю.
                ...
                and WHITHER in message and TIME in message \
                and SENDER in message and MESSAGE_TEXT in message \
                and self.names[message[SENDER]] == client:
            if message[WHITHER] in self.names:
    ...
    def autorize_user(self, message, sock):
        ...
        logger.debug(f'Запуск авторизации для: {message[USER]}')
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            ...
            try:
                logger.debug(f'Пользователь не зарегистрирован. {response}')
                send_message(sock, response)
            except OSError:
                pass
            self.clients.remove(sock)
            ...
            logger.debug('Правильное имя пользователя, '
                         'запуск проверки пароля.')
            # Иначе отвечаем 511 и проводим процедуру авторизации
            # Словарь - заготовка
            message_auth = RESPONSE_511
            # Набор байтов в hex представлении
            random_str = binascii.hexlify(urandom(64))
